...
if docker volume inspect todo-maven-repo &> /dev/null; then
    echo -e "${GREEN}Maven 缓存 volume 已存在。${NC}"
else
    echo -e "${GREEN}创建 Maven 缓存 volume...${NC}"
    docker volume create --name todo-maven-repo
fi
...
docker run -it --rm --name todo-maven \
    -v todo-maven-repo:/root/.m2 \
    -v "$PWD/todo-server":/usr/src/mymaven \
    -w /usr/src/mymaven \
...
if docker-compose ps | grep "todo"; then
